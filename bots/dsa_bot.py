...
from telebot.types import Message as TelegramMessage, CallbackQuery, InlineKeyboardMarkup, InlineKeyboardButton, WebAppInfo
...
async def lifespan(app: FastAPI):
    ...
    # Set webhook
    await dsa_bot.set_webhook(
        url=BOT_URL_BASE + DSA_BOT_TOKEN
    )
    yield
...
dsa_bot = async_telebot.AsyncTeleBot(DSA_BOT_TOKEN, state_storage=state_storage)
...
async def is_an_official(user_id) -> User | None:
    official_user_ref = db.collection(
...
async def send_welcome(message):
    ...
    if official_user:
        await dsa_bot.send_message(chat_id=message.from_user.id,
...
@dsa_bot.callback_query_handler(state=[UserState.message], func=lambda call: call.data.startswith("attach_file"))
...
@dsa_bot.message_handler(content_types=['audio', 'photo', 'voice', 'video', 'document'], state=UserState.attachments)
...
@dsa_bot.callback_query_handler(state=[UserState.attachments], func=lambda call: call.data.startswith("send_message"))
async def callback_query(call: CallbackQuery, state: StateContext):
    user_id = call.from_user.id
    ...
    if call.data == "send_message_yes":
        ...
        await dsa_bot.send_message(chat_id=chat_id,
